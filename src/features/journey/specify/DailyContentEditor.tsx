...
export function DailyContentEditor({ days, onChange }: { days: DailyContent[]; onChange: (days: DailyContent[]) => void }) {
  ...
  const fields: Array<{
    ...
    { key: 'content', label: 'เนื้อหาสั้น ๆ', question: 'ให้ความรู้หรือบริบทเท่าที่จำเป็นก่อนทำแบบฝึก', rows: 4 },
    { key: 'exercise', label: 'แบบฝึก', question: 'ผู้ใช้ต้องคิด เลือก หรือทำอะไรจริง', rows: 3 },
    { key: 'reflection', label: 'คำถามสะท้อนคิด', question: 'คำถามใดช่วยให้ผู้ใช้ตีความสิ่งที่เพิ่งทำ', rows: 2 },
    ...
    { key: 'content', label: 'SHORT CONTENT', question: 'Give only the context needed before the exercise.', rows: 4 },
    { key: 'exercise', label: 'EXERCISE', question: 'What will the user think, choose, or do?', rows: 3 },
    { key: 'reflection', label: 'REFLECTION', question: 'What question helps the user interpret the activity?', rows: 2 },
