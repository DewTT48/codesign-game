...
After receiving the file-creation command and completing all three sections, create a downloadable file named CODESIGN_SPEC.md. If this system cannot create files, return the entire Markdown in one code block so I can paste it into CODESIGN.`,
    ...
    PRD: {
      th: {
        ...
        chatGoal: 'ตรวจความสอดคล้องของชุดส่งต่องานทั้ง 4 ไฟล์ แล้วช่วยปิดเฉพาะจุดที่เปลี่ยน Product จริง',
        prompt: `ช่วยตรวจชุดส่งต่องานของ “21 DAYS OF ${topic}” ทั้ง 4 ไฟล์ในฐานะ Handoff reviewer\n\n===== CODESIGN_HANDOFF.md =====\n${text(current.handoff)}\n\n===== CONTENT_PACK.md =====\n${text(current.contentPack)}\n\n===== EXPERIENCE_DIRECTION.md =====\n${text(current.experienceDirection)}\n\n===== START_WITH_CODEX.md =====\n${text(current.startWithCodex)}\n\nบริบทสำคัญ:\n- ไฟล์ทั้ง 4 ถูก CODESIGN ประกอบจากการตัดสินใจที่ผู้ใช้ทำไว้ก่อนหน้า แต่ยังอาจเกิดข้อความไม่ตรงกันระหว่างแหล่งข้อมูลได้\n- แต่ละไฟล์มีหน้าที่ต่างกัน: CODESIGN_HANDOFF กำกับ Product และ Scope, CONTENT_PACK กำกับเนื้อหา 21 วัน, EXPERIENCE_DIRECTION กำกับภาพและประสบการณ์, START_WITH_CODEX กำกับวิธีเริ่มสร้าง\n\nวิธีตรวจ:\n1. อ่านทั้ง 4 ไฟล์ก่อนสรุป ห้าม Review เพียงไฟล์เดียว\n2. เปรียบเทียบข้อความข้ามไฟล์ โดยแยกให้ชัดว่าเป็นเพียงถ้อยคำต่างกันแต่ความหมายเดียวกัน หรือทำให้ผู้พัฒนาสร้าง Behavior ต่างกันจริง\n3. จัดประเภทเฉพาะประเด็นที่พบเป็น:\n   - ASSEMBLY MISMATCH — ไฟล์ที่ CODESIGN ประกอบขึ้นพูดไม่ตรงกัน\n   - OWNER DECISION CONFLICT — การตัดสินใจที่บันทึกไว้ขัดกันและเจ้าของ Product ต้องเลือก\n   - MISSING DECISION — ยังไม่มีคำตอบและคำตอบนั้นเปลี่ยน User, Journey, Content, Completion, Data หรือ Scope\n   - TESTABILITY — Requirement สำคัญยังตรวจ Pass/Fail ไม่ได้\n   - EDITORIAL ONLY — แก้ถ้อยคำได้โดยไม่เปลี่ยน Product\n4. อ้างชื่อไฟล์ หัวข้อ และข้อความที่ขัดกันทุกครั้ง ห้ามเดาว่าเกิดจากแหล่งใด\n5. อย่านับการเก็บคำตอบไว้เฉพาะ Browser ว่าเป็นการส่งหรือเก็บข้อมูลบน Server และอย่านับ Product principle เป็น Feature โดยอัตโนมัติ\n6. ห้ามเพิ่ม Feature, Product rule หรือ Solution ใหม่ และห้ามตัดสินใจแทนเจ้าของ Product\n\nรูปแบบการสนทนา:\n- รอบแรกแสดงเฉพาะประเด็นที่กระทบการสร้างสูงสุดไม่เกิน 5 ข้อ เรียงตามผลกระทบ\n- ในแต่ละข้อบอก WHY IT MATTERS, FILES AFFECTED และ QUESTION FOR OWNER สั้น ๆ\n- จากนั้นถามผมทีละหนึ่งคำถามและรอคำตอบ อย่าส่งรายงานยาวทั้งหมดในครั้งเดียว\n- เมื่อปิดทุกคำถามแล้ว สรุป EXACT EDITS แยกตามชื่อไฟล์ โดยเสนอเฉพาะข้อความที่ควรแทนที่ ห้ามสร้างไฟล์ฉบับใหม่จนกว่าผมจะยืนยัน`,
        followUps: ['ประเด็นใดเป็น Assembly mismatch ไม่ใช่ Product decision ใหม่?', 'ถ้าต้องปิดเพียงหนึ่งข้อ ข้อใดเปลี่ยน Behavior ของ App มากที่สุด?', 'หลังตอบครบแล้ว ช่วยสรุป Exact edits แยกตามไฟล์'],
        bringBack: 'ตอบคำถามทีละข้อ แล้วนำ Exact edits กลับมาแก้ในไฟล์ที่เกี่ยวข้องทั้ง 4 ไฟล์ก่อน Lock',
      },
      en: {
        ...
        chatGoal: 'Review consistency across all four handoff files and close only issues that materially change the product.',
        prompt: `Review all four handoff files for “21 DAYS OF ${topic}” as a handoff reviewer.\n\n===== CODESIGN_HANDOFF.md =====\n${text(current.handoff)}\n\n===== CONTENT_PACK.md =====\n${text(current.contentPack)}\n\n===== EXPERIENCE_DIRECTION.md =====\n${text(current.experienceDirection)}\n\n===== START_WITH_CODEX.md =====\n${text(current.startWithCodex)}\n\nImportant context:\n- CODESIGN assembled these files from earlier owner decisions, but assembly mismatches may still exist.\n- Each file has a distinct authority: CODESIGN_HANDOFF covers product and scope, CONTENT_PACK covers the 21-day content, EXPERIENCE_DIRECTION covers visual and interaction guidance, and START_WITH_CODEX covers the build-start workflow.\n\nReview method:\n1. Read all four files before reporting; do not review only one.\n2. Compare across files and distinguish harmless wording differences from differences that cause materially different behavior.\n3. Classify findings as ASSEMBLY MISMATCH, OWNER DECISION CONFLICT, MISSING DECISION, TESTABILITY, or EDITORIAL ONLY.\n4. Cite the file, section, and conflicting wording. Do not guess its origin.\n5. Do not treat browser-only saved responses as server collection, and do not automatically treat a product principle as a feature.\n6. Do not add features, product rules, or solutions, and do not decide for the owner.\n\nConversation format:\n- First show at most five highest-impact issues.\n- For each, state WHY IT MATTERS, FILES AFFECTED, and one short QUESTION FOR OWNER.\n- Then ask one question at a time and wait. Do not dump a full audit at once.\n- After all questions are resolved, provide EXACT EDITS grouped by filename. Do not regenerate the files until I approve.`,
        followUps: ['Which issue is an assembly mismatch rather than a new product decision?', 'Which single issue would most change app behavior?', 'After the decisions are closed, list exact edits by filename.'],
        bringBack: 'Answer one question at a time, then apply the exact edits to all affected files before locking.',
      },
    },
    I: {
      th: {
