...
const fieldGuides: Record<string, Localized<FieldGuide>> = {
  ...
  'implement.repoUrl': field('ใส่ Repository ที่เป็น source ของ Build นี้ หากต้องการเก็บใน Project Record', 'Provide the source repository for this build if it should appear in the Project Record', 'https://github.com/username/project', 'https://github.com/username/project'),
...
After receiving the file-creation command and completing all three sections, create a downloadable file named CODESIGN_SPEC.md. If this system cannot create files, return the entire Markdown in one code block so I can paste it into CODESIGN.`,
    ...
    PRD: {
      th: {
        ...
        hint: 'หลังจากคุณยืนยัน Step S แล้ว CODESIGN ได้สร้างไฟล์ร่าง 3 ฉบับจากคำตอบและการตัดสินใจที่ยืนยันไว้ตั้งแต่ Step C–S โดยอัตโนมัติ ในการตรวจรอบแรกนี้ คุณไม่ต้องสร้างหรืออัปโหลดไฟล์เพิ่ม',
      ...
      en: {
        ...
        hint: 'After you confirmed Step S, CODESIGN automatically created three draft files from the answers and decisions confirmed across Steps C–S. You do not need to create or upload anything for this first review.',
    ...
    I: {
      th: {
        ...
        bringBack: 'บันทึกเฉพาะ Public App URL หลัง Build ทำงานจริงและผ่านการ Preview ไม่ต้องนำ Repository URL กลับมาใส่ใน CODESIGN',
      },
      en: {
        ...
        hint: 'Create an app folder, download all four handoff files, move them from Downloads into that folder without renaming them, and open it as a local project in Codex before pasting the prompt into a new task.',
        chatGoal: 'These instructions are specifically for Codex. They hand off the Build Package with GitHub readiness and explicit decision boundaries.',
        prompt: `Use these instructions in Codex.\n\nBuild “21 DAYS OF ${topic}” from the four handoff files in the current project folder, guiding me step by step as a non-technical owner.\n\nCommunicate, explain progress, and ask me questions in English unless I request another language. Keep filenames, code, terminal commands, and technical identifiers in their original form.\n\nGITHUB READINESS: ${text(current.githubReadiness)}\n\nRules:\n1. Read CODESIGN_HANDOFF.md, CONTENT_PACK.md, EXPERIENCE_DIRECTION.md, and START_WITH_CODEX.md from this project folder. Do not ask me to attach them again.\n2. Treat them as the source of truth. Do not add features outside Must Have.\n3. Decide code structure, responsive layout, and implementation details that do not change the product.\n4. If ambiguity changes the user, journey, content, completion rule, data behavior, or experience direction, mark PRODUCT DECISION REQUIRED and ask one question.\n5. Explain GitHub, repositories, and GitHub Pages plainly; guide account and repository setup if needed.\n6. Never request passwords, OTPs, CAPTCHAs, recovery codes, or 2FA secrets. Pause while I complete security steps.\n7. Build all 21 days and test desktop, tablet, mobile, keyboard use, and text wrapping.\n8. Show me a preview before publishing through GitHub Pages.\n\nFirst summarize the build, non-goals, my GitHub readiness, and only genuine product-decision questions.`,
        ...
        bringBack: 'Save only the public app URL after the build works and has been previewed. Do not enter the repository URL in CODESIGN.',
      },
    },
    G: {
      th: {
  ...
  const alignmentExtensions: Record<string, Localized<{ prompt: string; bringBack?: string }>> = {
    ...
    I: {
      th: {
        ...
        bringBack: 'บันทึกเฉพาะ Public App URL และสถานะ PRD → I พร้อมคำอธิบายหลัง Build ผ่านการทดสอบ ไม่ต้องบันทึก Repository URL',
      },
      en: {
        ...
        bringBack: 'Save only the public app URL plus the PRD → I status and explanation after the tested build. Do not save the repository URL.',
      },
    },
    G: {
      th: {
