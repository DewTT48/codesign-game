...
After receiving the file-creation command and completing all three sections, create a downloadable file named CODESIGN_SPEC.md. If this system cannot create files, return the entire Markdown in one code block so I can paste it into CODESIGN.`,
    ...
    PRD: {
      th: {
        ...
        chatGoal: 'ตรวจความสอดคล้องของไฟล์หลัก 3 ฉบับ ปิดเฉพาะจุดที่เปลี่ยน Product จริง แล้วสร้างไฟล์ฉบับเต็มกลับมาให้ตรวจ',
        prompt: `ช่วยตรวจไฟล์หลักของ “21 DAYS OF ${topic}” ทั้ง 3 ไฟล์ในฐานะ Handoff reviewer\n\n===== LOCKED OWNER DECISIONS =====\nWHO: ${who}\nGOAL: ${goal}\nSUCCESS: ${success}\nCONTEXT: ${context}\nCONSTRAINTS: ${constraints}\nDIRECTION: ${direction}\nMUST HAVE: ${mustHaves}\nNOT IN THIS VERSION: ${nonGoals}\nPRIMARY JOURNEY: ${sourceValue(source, 'S', 'journeySummary')}\nONE DAY IS COMPLETE WHEN: ${sourceValue(source, 'S', 'dailyCompletionRule')}\nRETURN RULE: ${sourceValue(source, 'S', 'returnRule')}\nSEQUENCE RULE: ${sourceValue(source, 'S', 'sequenceRule')}\nSTORAGE RULE: ${sourceValue(source, 'S', 'storageRule')}\nPRODUCT LANGUAGE: ${sourceValue(source, 'S', 'productLanguage')}\n\n===== CODESIGN_HANDOFF.md =====\n${text(current.handoff)}\n\n===== CONTENT_PACK.md =====\n${text(current.contentPack)}\n\n===== EXPERIENCE_DIRECTION.md =====\n${text(current.experienceDirection)}\n\nบริบทสำคัญ:\n- ไฟล์ทั้ง 3 ถูก CODESIGN ประกอบจาก Locked Owner Decisions ด้านบน ให้ถือการตัดสินใจเหล่านั้นเป็นหลักเมื่อตรวจ Assembly mismatch\n- CODESIGN_HANDOFF กำกับ Product และ Scope, CONTENT_PACK กำกับเนื้อหา 21 วัน, EXPERIENCE_DIRECTION กำกับภาพและประสบการณ์\n- START_WITH_CODEX.md ยังไม่ถูกสร้างในขั้นนี้ เพราะต้องรอผู้ใช้เลือกระดับความพร้อมเรื่อง GitHub ใน Stage Implement\n\nวิธีตรวจ:\n1. อ่าน Locked Owner Decisions และทั้ง 3 ไฟล์ก่อนสรุป ห้าม Review เพียงไฟล์เดียว\n2. แยกถ้อยคำต่างกันแต่ความหมายเดียวกัน ออกจากความต่างที่ทำให้ผู้พัฒนาสร้าง Behavior ต่างกันจริง\n3. จัดประเภทเฉพาะประเด็นที่พบเป็น ASSEMBLY MISMATCH / OWNER DECISION CONFLICT / MISSING DECISION / TESTABILITY / EDITORIAL ONLY\n4. อ้างชื่อไฟล์ หัวข้อ และข้อความที่เกี่ยวข้องทุกครั้ง ห้ามเดาที่มาหรือยก implementation preference เป็น Product decision\n5. ห้ามเพิ่ม Feature, Product rule หรือ Solution ใหม่ และห้ามตัดสินใจแทนเจ้าของ Product\n\nรูปแบบการสนทนา:\n- รอบแรกแสดงเฉพาะประเด็นที่กระทบการสร้างสูงสุดไม่เกิน 5 ข้อ เรียงตามผลกระทบ\n- ในแต่ละข้อบอก WHY IT MATTERS, FILES AFFECTED และ QUESTION FOR OWNER สั้น ๆ\n- จากนั้นถามผมทีละหนึ่งคำถามและรอคำตอบ\n- เมื่อปิดทุกคำถามแล้ว สรุป EXACT EDITS แยกตามชื่อไฟล์ และรอให้ผมยืนยัน\n- เมื่อผมพิมพ์ UPDATE HANDOFF FILES ให้สร้างไฟล์ฉบับเต็มล่าสุดทั้ง 3 ไฟล์ตามชื่อ CODESIGN_HANDOFF.md, CONTENT_PACK.md และ EXPERIENCE_DIRECTION.md แม้บางไฟล์ไม่มีการเปลี่ยนแปลง\n- ห้ามสร้าง START_WITH_CODEX.md และห้ามเปลี่ยนชื่อไฟล์ หากสร้างไฟล์ดาวน์โหลดไม่ได้ ให้คืน Markdown ฉบับเต็มเป็น 3 code blocks ที่มีชื่อไฟล์กำกับชัดเจน`,
        ...
        bringBack: 'ตอบคำถามทีละข้อ ยืนยัน Exact edits แล้วพิมพ์ UPDATE HANDOFF FILES จากนั้นอัปโหลดไฟล์ฉบับเต็มทั้ง 3 กลับเข้า CODESIGN',
      },
      en: {
        ...
        chatGoal: 'Review consistency across the three source files, close only material product issues, then return complete updated files for confirmation.',
        prompt: `Review the three source files for “21 DAYS OF ${topic}” as a handoff reviewer.\n\n===== LOCKED OWNER DECISIONS =====\nWHO: ${who}\nGOAL: ${goal}\nSUCCESS: ${success}\nCONTEXT: ${context}\nCONSTRAINTS: ${constraints}\nDIRECTION: ${direction}\nMUST HAVE: ${mustHaves}\nNOT IN THIS VERSION: ${nonGoals}\nPRIMARY JOURNEY: ${sourceValue(source, 'S', 'journeySummary')}\nONE DAY IS COMPLETE WHEN: ${sourceValue(source, 'S', 'dailyCompletionRule')}\nRETURN RULE: ${sourceValue(source, 'S', 'returnRule')}\nSEQUENCE RULE: ${sourceValue(source, 'S', 'sequenceRule')}\nSTORAGE RULE: ${sourceValue(source, 'S', 'storageRule')}\nPRODUCT LANGUAGE: ${sourceValue(source, 'S', 'productLanguage')}\n\n===== CODESIGN_HANDOFF.md =====\n${text(current.handoff)}\n\n===== CONTENT_PACK.md =====\n${text(current.contentPack)}\n\n===== EXPERIENCE_DIRECTION.md =====\n${text(current.experienceDirection)}\n\nUse the locked owner decisions as the authority when checking assembly mismatches. Read all three files, distinguish harmless wording differences from behavior-changing conflicts, and classify findings as ASSEMBLY MISMATCH, OWNER DECISION CONFLICT, MISSING DECISION, TESTABILITY, or EDITORIAL ONLY. Cite the affected file and section. Do not invent features, product rules, or solutions, and do not turn implementation preferences into owner decisions.\n\nFirst show at most five highest-impact issues. For each, state WHY IT MATTERS, FILES AFFECTED, and one short QUESTION FOR OWNER. Then ask one question at a time and wait. After all questions are closed, provide EXACT EDITS by filename and wait for approval. When I type UPDATE HANDOFF FILES, return complete current versions of CODESIGN_HANDOFF.md, CONTENT_PACK.md, and EXPERIENCE_DIRECTION.md, including unchanged files. Do not create START_WITH_CODEX.md or rename files. If downloads are unavailable, return three complete fenced Markdown blocks labeled with the exact filenames.`,
        ...
        bringBack: 'Answer one question at a time, approve the exact edits, type UPDATE HANDOFF FILES, then upload all three complete files to CODESIGN.',
      },
    },
    I: {
      th: {
