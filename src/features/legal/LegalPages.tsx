...
export function PrivacyPage() {
  ...
  if (!isThai) {
    return (
      <LegalLayout code="DATA & PRIVACY" title="PRIVACY POLICY" summary="CODESIGN collects only the information needed for sign-in, saving your product journey, and continuing your work later.">
        ...
        <section><h2>2. HOW THE INFORMATION IS USED</h2><ul><li>Authenticate you and keep each user&apos;s projects separate.</li><li>Save and restore your Guided Build journey.</li><li>Assemble the PRD and CODESIGN Journal from your decisions.</li><li>Maintain security and troubleshoot the service.</li><li>Provide authorized administrators with aggregate usage and account metadata needed to operate the service.</li></ul></section>
        ...
        <section><h2>4. STORAGE & SECURITY</h2><p>Project data is stored in Supabase and protected by Row Level Security so a signed-in account can read and change only its own records. The publishable key in the frontend cannot bypass these policies.</p><p>The read-only Admin Dashboard shows authorized administrators account metadata, mission counts, status, and aggregate progress. It does not expose users&apos; answers, Decisions, PRDs, or Journals.</p></section>
  ...
  return (
    <LegalLayout code="DATA & PRIVACY" title="PRIVACY POLICY" summary="CODESIGN เก็บข้อมูลเท่าที่จำเป็นเพื่อให้ผู้ใช้เข้าสู่ระบบ บันทึก Product journey และกลับมาทำงานต่อได้">
      ...
      <section><h2>2. HOW THE INFORMATION IS USED</h2><ul><li>ยืนยันตัวตนและแยก Project ของผู้ใช้แต่ละคน</li><li>บันทึกและกู้คืน Guided Build journey</li><li>ประกอบ PRD และ CODESIGN Journal ตามการตัดสินใจของผู้ใช้</li><li>รักษาความปลอดภัยและแก้ปัญหาการทำงานของบริการ</li><li>แสดงข้อมูลการใช้งานโดยรวมและข้อมูลพื้นฐานของบัญชีแก่ผู้ดูแลระบบที่ได้รับสิทธิ์ เพื่อใช้ดูแลบริการ</li></ul></section>
      ...
      <section><h2>4. STORAGE & SECURITY</h2><p>ข้อมูล Project ถูกจัดเก็บใน Supabase และป้องกันด้วย Row Level Security เพื่อให้บัญชีที่เข้าสู่ระบบอ่านและแก้ไขได้เฉพาะข้อมูลของตนเอง Publishable key ที่อยู่ใน Frontend ไม่สามารถข้ามนโยบายนี้ได้</p><p>Admin Dashboard แบบอ่านอย่างเดียวจะแสดงข้อมูลพื้นฐานของบัญชี จำนวน Mission สถานะ และ Progress โดยรวมแก่ผู้ดูแลระบบที่ได้รับสิทธิ์ แต่จะไม่แสดงคำตอบ Decisions, PRD หรือ Journal ของผู้ใช้</p></section>
