...
begin
  ...
  select
    ...
    field_key,
    case
      when phase = 'S' and field_key in ('contentOwnerConfirmed', 'experienceOwnerConfirmed') then 'false'::jsonb
      when phase = 'PRD' and field_key in ('confirmedFiles', 'confirmedFilesV2') then '[]'::jsonb
      when phase = 'I' and field_key = 'workingApp' then 'false'::jsonb
      when phase = 'G' and field_key in ('mobile', 'start', 'dailyFlow', 'saveData', 'reopen', 'persistence', 'navigation', 'prdRules') then 'false'::jsonb
      else content
    end,
    'captured',
    version + 1,
